...
@dataclass
class Wav2vec_U_LM_Config(FairseqDataclass):
...
class Discriminator(nn.Module):
    def __init__(self, dim, cfg: Wav2vec_U_LM_Config):
        super().__init__()
...
class Generator(nn.Module):
    def __init__(self, input_dim, output_dim, cfg: Wav2vec_U_LM_Config):
        super().__init__()
...
@register_model("wav2vec_u_lm", dataclass=Wav2vec_U_LM_Config)
class Wav2vec_U_LM(BaseFairseqModel):
    ...
    def __init__(self, cfg: Wav2vec_U_LM_Config, target_dict):
        super().__init__()
    ...
    def forward(
        ...
        gen_result = self.generator(features, None, padding_mask)
        ...
        start_tokens = torch.ones((sample_size, 1), dtype=torch.long, device=features.device) * self.zero_index
        # start_tokens = torch.randint(4, self.ref_lm.config.vocab_size, (sample_size, 1))
        with torch.no_grad():
            # forbid_tokens_mask = torch.zeros((1, self.ref_lm.config.vocab_size), device=features.device)
            # forbid_tokens_mask[:, :4] = -1e6 # Forbid tokens not present in the corpus
            # token_x = self.ref_lm.generate(start_tokens, 
            #                                self.ref_lm.config.block_size // 2 - 1,
            ...
            token_x = token_x[:, :token_x_lens.max()]
            token_padding_mask = torch.arange(token_x.size(1), device=token_x.device).unsqueeze(0) >= token_x_lens.unsqueeze(1)
            token_x[token_padding_mask] = self.pad
        # token_padding_mask = torch.zeros_like(token_x, dtype=torch.bool)
        token_x = F.one_hot(token_x, num_classes=self.ref_lm.config.vocab_size).float()
