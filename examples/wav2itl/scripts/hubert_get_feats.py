        ...
        def iterate():
            for fname in files:
                ...
                with torch.no_grad():
                    ...
                    audio_rep = mdl(source=wav, mask=False, features_only=True, output_layer=cfg.model.encoder_layers)['layer_results']
                    audio_rep = [rep[0] for rep in audio_rep]
                    audio_rep = torch.concatenate(audio_rep, dim=1).cpu().numpy()
                yield audio_rep
    return iterate, num
